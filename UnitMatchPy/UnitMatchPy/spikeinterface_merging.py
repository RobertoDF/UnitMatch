...
from .spikeinterface_diagnostics import (
    _UnitDiagnostics,
    _load_unit_diagnostics,
    _probe_plot_axes,
...
class SpikeInterfaceSessionMerger:
    ...
    def _get_unit_diagnostics(self, unit_id):
        return _load_unit_diagnostics(self._unit_sources[unit_id], unit_id)
    ...
    def _make_group_figure(self, group):
        ...
        def update_waveforms_for_interval(time_start_s, time_stop_s):
            ...
            selected_waveforms, selected_counts = _waveforms_for_interval(
                diagnostics,
                waveform_channel_indices,
                (time_start_s, time_stop_s),
            )
            for diagnostic_index, unit_id in enumerate(group):
                for axis_index in range(len(waveform_channel_indices)):
                    line = waveform_lines.get((axis_index, unit_id))
                    waveform = selected_waveforms.get(
                        (axis_index, diagnostic_index)
                    )
                    if line is not None and waveform is not None:
            ...
            selection_title.set_text(
                ...
                f"({', '.join(f'{unit_id}: {selected_counts[index]}' for index, unit_id in enumerate(group))} "
                "sampled spikes); right-click to reset"
            )
        ...
        def reset_waveforms(event=None):
            for patch in selection_patches:
                patch.remove()
            selection_patches.clear()
            reset_waveforms, _ = _waveforms_for_interval(
                diagnostics,
                waveform_channel_indices,
            )
            for diagnostic_index, unit_id in enumerate(group):
                for axis_index in range(len(waveform_channel_indices)):
                    line = waveform_lines.get((axis_index, unit_id))
                    waveform = reset_waveforms.get((axis_index, diagnostic_index))
                    if line is not None and waveform is not None:
                        line.set_ydata(waveform)
            for waveform_axis in waveform_axes:
                waveform_axis.relim()
    ...
    def apply_and_save(
        ...
        self.apply_merges(job_kwargs=job_kwargs)
